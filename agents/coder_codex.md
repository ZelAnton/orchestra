...
подбери записи типа `pitfall` с `confidence: med|high`, чей нормализованный `scope_file`
пересекается с областью задачи (для Режима 1 — из `Конфликт-домен`/`task.md`; для Режима 2 —
из файлов находок), и добавь их в промпт кратким блоком `Known pitfalls in this area (avoid
repeating):` (по строке на ловушку). Перед сравнением `scope` вычисли `scope_file` как
подстроку scalar `scope` до первого `::`: для обычного файла/каталога это весь `scope`, для
`file::SymbolName` и `file::heading:<...>` только `file`; используй только `scope_file` для
path intersection, а anchored-суффикс — только для проверки якоря. Если точный якорь нужен,
