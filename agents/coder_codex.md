...
**Foreground-инвариант (граница разрешений).** Все вызовы `codex-runtime.ps1 run` и
`resume-image` выполняй одним foreground Bash-вызовом до завершения. Не добавляй `&`,
...
Передавай `--timeout-sec 1800` и задай timeout Bash tool call не меньше 1 900 000 ms.
