...
1. `RUNTIME_LAYOUT=checkout` → подставляй
   ...
   launcher'ов и seed-правилом `cc-config`).
...
Для каждого ключа ниже используй единый порядок: `$WORK/config.md` →
`$HOME/.orchestra/root-config.md` → указанный default. Системное окружение не является
источником конфигурации.
...
- `CODEX_CODER_DEEP_MODEL` (по умолч. `gpt-5.6-sol`) — модель **этой** роли для уровня
  ...
  Для обоих модельных ключей действует тот же общий порядок разрешения выше.
...
Используй переданный processor'ом `VCS`; не переопределяй его probe-командой. При
`VCS=jj` запрещены ручные git-команды в `$WT`: pure-jj workspace иначе молча адресует
родительский `.git`. При `VCS=git` runtime/guard обязан подтвердить, что Git top-level
равен `$WT`, иначе верни `CODEX_UNAVAILABLE` как ошибку адресации. Зафиксируй
...
- **Режим 1** (реализация): прочитай `$WORK/tasks/<T-ID>/task.md`, вложи в промпт
  ...
  Если descriptor содержит metadata-строку `Ограничение радиуса:`, prompt builder обязан
  передать её целиком, без пересказа или сужения, и сохранить её требование править
  минимально в указанном файле, символе или заголовке без расширения diff без отдельного
  критерия выполнения. Если строки нет (в том числе при `KB=off`), не добавляй её и не
  выдумывай ограничение.
...
подбери записи типа `pitfall` с `confidence: med|high`, чьи нормализованные `scope_paths`
пересекаются с областью задачи (для Режима 1 — из `Конфликт-домен`/`task.md`; для Режима 2 —
из файлов находок), и добавь их в промпт кратким блоком `Known pitfalls in this area (avoid
repeating):` (по строке на ловушку). Перед сравнением `scope` раздели scalar по запятым,
отбрось пустые компоненты и trim-ь каждый путь; сравнивай каждый компонент с областью по
существующим правилам exact-file, границы каталога и glob. Если отдельный компонент имеет
anchored-форму `file::SymbolName` или `file::heading:<...>`, для path intersection возьми
часть до первого `::`, а суффикс оставь только для проверки якоря; у одиночного компонента
без `::` полный путь — это `scope_file`. Многопутевые и glob scope не сворачивай в одну
строку: они остаются широкими, но релевантными, и никогда не дают `Ограничение радиуса:`.
Если точный якорь нужен,
сверяй файл и `::SymbolName`/`::heading:<...>` с переданным committed `BASE` через VCS, а не
только с live worktree; без подтверждения в `BASE` якорь не считай валидным и не выдумывай
замену. `architecture`/`convention`-факты codex не навязывай — они не авторитетны и требуют
сверки с кодом. При `KB=off` или отсутствии каталога — пропусти.
...
- If a required step needs network that fails in this sandbox (e.g. `cargo fetch`/`cargo
  update`, `dotnet restore`, `dotnet tool restore`, or any dependency download), do NOT keep retrying it. Finish immediately with a
  ...
  `pip install -r <file>`, `pip download`, `uv lock`, `uv sync`, `dotnet tool restore`,
  `dotnet restore` or `dotnet restore <relative project/solution path>`. A broker outside
  the sandbox runs it and restarts you; anything outside this list is rejected.
...
- `on` (дефолт) — сеть открыта, но **не для всех** инструментов; дословно (смысл сохрани):
  ...
  - Network access IS available, but only for OpenSSL-based tooling: node/npm, python/pip
    ...
    or Windows certificate/repository-signature services — notably cargo, NuGet/dotnet and
    a default (non-openssl) git — may NOT work in this sandbox even with the network on;
    do NOT retry their network steps here, leave them to the broker (signal them with the
    NEED_NET line below).
  ```
...
**Foreground-инвариант (граница разрешений).** Все вызовы `codex-runtime.ps1 run`,
`resume-image` и `broker-run` выполняй одним foreground Bash-вызовом до завершения. Не добавляй `&`,
`& disown`, `nohup`, `Start-Process`, shell-job или `sleep`+polling: background-форма
намеренно требует отдельного approval, даже когда foreground-префикс runtime предвыдан.
Для `run`/`resume-image` передавай `--timeout-sec 1800` и задай timeout Bash tool call не
меньше 1 900 000 ms; для `broker-run` — `--timeout-sec 900` и не меньше 1 000 000 ms.
Не удалось выполнить foreground — верни `CODEX_UNAVAILABLE`, не обходи границу фоном.
...
Единственный источник истины по состоянию рабочей копии — runtime с уже выбранным
`$VCS`:

`pwsh -File $CODEX_RT working-copy-status --worktree "$WT" --vcs "$VCS"`

Он возвращает JSON `{vcs, clean, changedFiles, stat}`. Команда **требует** явный
`--vcs` и ветвится внутри runtime: при `VCS=jj` вызывает только
`jj -R "$WT" diff --name-only/--stat`, при `VCS=git` — только git-команды. Не
подменяй этот вызов ручным `git status`/`git diff` и не выбирай git из-за наличия
`.git`: в colocated jj+git рабочая копия может быть содержательно грязной по
`jj diff`, когда Git уже показывает clean. Явный `-R "$WT"` внутри runtime обязателен —
без него jj молча покажет/снапшотит рабочую копию cwd, а не `$WT` (K-025).

По `changedFiles` строишь `Изменённые файлы:` и проверяешь, что находки Режима 2
действительно дали правки. `clean=true` при непустой задаче → codex ничего не сделал →
сбой. Если runtime-проверка сама не выполнилась, состояние **неизвестно**: запрещено
писать «working copy clean»/«изменений нет».
...
3. Постфактум — это **делает runtime**, а не ты руками: зафиксируй исходную вершину `PRE`
   ...
   `pwsh -File $CODEX_RT guard-commit --worktree "$WT" --vcs "$VCS" --pre "$PRE" --reset`.
...
|---|---|---|
| `sandbox-init-worktree` | `windows unelevated restricted-token sandbox cannot enforce split writable root sets directly` | Windows sandbox отверг именно форму task worktree/дополнительных writable roots; это не хостовый отказ reviewer/main-tree вызовов |
...
| `profile-denied` | `Permission denied` на части профиля (`~/.config/git/ignore`, `~/.ssh/config`) | часть профиля пользователя недоступна; writable tool caches/TEMP runtime заранее перенаправляет в `<WT>/.work/codex-cache`; `workspace-write` уже разрешает этот путь, а `--add-dir` добавляется только для `read-only`, поэтому coder не создаёт лишний Windows writable root |
...
**Fail-closed при отказе инициализации песочницы (`sandbox-init`/
`sandbox-init-worktree`).** `CreateProcessAsUserW
...
`WT` **вне** песочницы codex через `codex-runtime.ps1 broker-run`. Смысл: у
адаптера рабочий TLS и сеть, а у песочницы Windows — restricted token, где schannel мёртв
(`SEC_E_NO_CREDENTIALS`) и NuGet может падать на TLS/проверке подписи репозитория, поэтому
cargo, NuGet/dotnet и schannel-git не тянут зависимости даже при `CODEX_NETWORK: on`.
Брокер регенерирует lock-файлы и наполняет разделяемый кэш (`~/.cargo`, npm/pip/NuGet
cache), который песочница codex затем **читает** для офлайн-сборки.
...
| .NET/NuGet | `.config/dotnet-tools.json`, `Directory.Packages.props`, `packages.lock.json`, `*.sln[x]`, `*.*proj` | `dotnet tool restore`, `dotnet restore`, `dotnet restore <относительный путь к .sln/.slnx/*proj>` |

Таблица **расширяема** (той же природы — регенерация lock + наполнение кэша), но пополнять её
может только правка **этого файла и исполняемого allowlist в `codex-runtime.ps1` в одном
изменении**, не codex. Чего в таблице runtime нет — брокер не выполняет. Аргументы .NET
намеренно узкие: `dotnet tool restore` без добавочных флагов; `dotnet restore` без аргументов
либо с одним относительным путём к `.sln`/`.slnx`/`.csproj`/`.fsproj`/`.vbproj`, без `..`,
MSBuild-свойств и CLI-флагов.
...
- **(а) Детерминированная постлюдия — основной механизм.** После прогона codex, если
  **одновременно** (1) в diff изменён манифест зависимостей (см. таблицу) и (2) вывод codex
  ...
  таблицы по экосистеме, выполняет их в `WT` вне песочницы **только через `broker-run`** и
  повторяет провалившийся гейт:
  ...
  влияет**, поэтому подстановка произвольной команды невозможна. Для .NET выбирай
  `dotnet tool restore`, если затронут `.config/dotnet-tools.json`; для package/project
  manifest — `dotnet restore` из корня, а если корень неоднозначен — форму с одним
  относительным project/solution path из allowlist.
- **(б) Явный протокол `NEED_NET:` — дополнение.** В промпт codex добавлена инструкция:
  ...
  Получив её, **валидацию и исполнение выполняет runtime в коде одним вызовом**:
  `pwsh -File $CODEX_RT broker-run --worktree "$WT" --timeout-sec 900 --command
  "<строка codex>"` → JSON `{ok, allowed, canonical, exitCode, timedOut, stdout, stderr}`.
  Он (1) отклоняет строку с метасимволами оболочки (`;`,
  `&&`, `||`, `|`, `` ` ``, `$(`, `>`, `<`, перевод строки); (2) требует, чтобы пара
  `<инструмент> <подкоманда>` была в allowlist; (3) допускает только канонические для операции
  аргументы (имя crate для `cargo update -p`, файл для `pip install -r`, безопасный
  относительный .NET project/solution path), прочее — отклоняет; (4) запускает уже
  провалидированные tool/args как argv без shell-интерполяции, с тем же timeout и очисткой
  всего дерева процессов, что и `codex exec`. **Никогда** не исполняй ни сырую строку codex,
  ни `canonical` отдельной Bash-командой: авторитет исполнения — только `broker-run`.

  Не прошло → **эскалация** `ЭСКАЛАЦИЯ codex: CODEX_FAILED — ENV_LIMIT/network: NEED_NET вне
  allowlist: <команда>` (правки codex отбрось, как при любом `CODEX_FAILED`). `allowed=true`,
  но `ok=false`/timeout/ненулевой exit → эскалация с короткой причиной из результата.
  `ok=true` → перезапусти codex. (а) предпочтительнее —
...
вносят сами lock/fetch-команды (`Cargo.lock`, `package-lock.json`, `uv.lock`,
`packages.lock.json` и т.п.), —
...
- `ЭСКАЛАЦИЯ codex: CODEX_FAILED — <причина>` — codex отработал, но не довёл (smoke не
  ...
  (`<класс>` ∈ `sandbox-init-worktree` | `sandbox-init` | `network` | `tls-schannel` | `vcs-write` | `profile-denied` |
...
- **worktree** (Режимы 1/2, а также Режим 3 в `_integration`):
  `pwsh -File $CODEX_RT cleanup --worktree "$WT" --vcs "$VCS"` — git
...
После `cleanup` и перед финальным отчётом **обязательно** повтори
`working-copy-status --worktree "$WT" --vcs "$VCS"`. Только его `clean=true` разрешает
написать, что рабочая копия очищена; `clean=false` требует перечислить оставшиеся
`changedFiles` и явно сообщить, что cleanup не завершён. Ошибка проверки означает
«состояние неизвестно», а не «clean». При `VCS=jj` запрещён контрольный
`git status`/`git diff` даже в colocated-репозитории.

При `CODEX_UNAVAILABLE` откатывать нечего; не добавляй к сентинелу утверждение о
чистоте, если `working-copy-status` не выполнялся.
...
Подтверждённую внешнюю проблему или возможность можно передать зарегистрированному
проекту через `inbox.ps1 send` по `$HOME/.orchestra/specs/Inbox_Contract.md`, выбрав
адресата через `project-registry.ps1 list`. Это не заменяет твой штатный fallback/
sentinel и локальную защиту, не разрешает менять чужой репозиторий и не должно задерживать
возврат processor. Передай факты, влияние, желаемый результат и альтернативы; укажи
`msg-id`.

# Финальный отчёт (возврат processor)

- Что реализовано (Режим 1) / какие `R-` устранены или отклонены и почему (Режим 2).
- `Изменённые файлы: <changedFiles из working-copy-status>`.
