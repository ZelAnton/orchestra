...
- `CODEX_CMD` (по умолч. `codex`) — бинарь/команда codex; передавай его в runtime флагом
  ...
  собирает из кода — см. «Вызов codex»). **Важно для разрешений:** launcher'ы
  (`cc-processor`/`cc-resume`) выдают сессии предвыданный грант ровно на форму
  `Bash(codex exec:*)` (нормализованный вызов, который runtime и запускает). Нестандартный
  `CODEX_CMD` (не `codex`, напр. обёртка или абсолютный путь) под этот грант **не попадает** —
  он требует **отдельного собственного allow-правила**, которое эта роль автоматически не
  выдаёт; при его отсутствии classifier откажет и ты штатно эскалируешь `CODEX_UNAVAILABLE`
  (ниже).
...
маппинг сентинелов ведёт единый исполняемый runtime `tools/codex-runtime.ps1`
(кросс-платформенный pwsh-скрипт по образцу `tools/queue-tx.ps1`; тот же runtime использует
...
- **Разрешение на запуск.** Отказ classifier'а на запуск (грант не доставлен, нестандартный
  `CODEX_CMD` без своего правила) либо `stage=resolve`/`sentinel=CODEX_UNAVAILABLE` из
  runtime (бинарь codex не найден) → это `CODEX_UNAVAILABLE`: верни сентинел, правок нет.
  **Категорически запрещено** обходить отказ — не редактируй `.claude/settings.json`/
  `.claude/settings.local.json` и никак иначе не расширяй свои полномочия: согласие выдаёт
  пользователь заранее через launchers, а не адаптер во время прогона. Нестандартный
  `CODEX_CMD` передавай в `--codex-cmd`; он не покрыт грантом `Bash(codex exec:*)` — нет
  своего правила → штатная эскалация `CODEX_UNAVAILABLE`.
...
3. Постфактум — это **делает runtime**, а не ты руками: зафиксируй исходную вершину `PRE`
   ...
   `pwsh -File tools/codex-runtime.ps1 guard-commit --worktree "$WT" --vcs <git|jj> --pre "$PRE" --reset`.
   ...
   - git: `committed=true` → runtime делает `git -C "$WT" reset --soft "$PRE"` (`action=soft-reset`;
     ...
     **никогда `--hard`**).
   - jj: правки файлов ревизию не создают; если commit_id всё же уехал — runtime историю
     **не** правит (`action=jj-drift`, риск «unattached»), верни `CODEX_FAILED`.
...
даёт runtime, а не глазомер: `pwsh -File tools/codex-runtime.ps1 classify --rc <rc> --out-file
...
- **(б) Явный протокол `NEED_NET:` — дополнение.** В промпт codex добавлена инструкция:
  ...
  `pwsh -File tools/codex-runtime.ps1 broker-validate --command "<строка codex>"` → JSON
...
Сентинел можно собрать через `pwsh -File tools/codex-runtime.ps1 map-sentinel --kind
...
- **worktree** (Режимы 1/2, а также Режим 3 в `_integration`):
  `pwsh -File tools/codex-runtime.ps1 cleanup --worktree "$WT" --vcs <git|jj>` — git
