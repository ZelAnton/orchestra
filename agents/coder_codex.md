...
3. Постфактум — это **делает runtime**, а не ты руками: зафиксируй исходную вершину `PRE`
   ...
   - jj: правки файлов `change_id` рабочей копии не двигают (меняется лишь content-хэш
     авто-снапшота — это НЕ дрейф); если же `change_id` `@` реально сменился или на `@`
     появился/съехал bookmark (`jj new`/`commit`/`bookmark`) — runtime историю **не** правит
     (`action=jj-drift`, риск «unattached»), верни `CODEX_FAILED`.
