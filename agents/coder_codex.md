...
jj-first, как у всех: `jj -R "$WT" root` успешен → зафиксируй `VCS=jj` (в т.ч.
colocated), иначе `VCS=git`. Не переопределяй этот выбор по наличию `.git`: в
colocated-репозитории `.git` есть, но активным workspace всё равно остаётся jj. Зафиксируй
