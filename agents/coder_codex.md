...
- **Fail-closed режим одобрения (`-c approval_policy=never`) — обязателен на КАЖДОМ вызове.**
  Это Orchestra-фиксируемая политика, а не пользовательская: она **не** зависит от значения
  `approval_policy` в `~/.codex/config.toml` (там дефолт может быть `on-request`/`on-failure`,
  который при отказе песочницы эскалирует команду к запуску **без** неё). Под `never` codex
  **никогда** не повышает режим исполнения: команду, которую не удалось выполнить в песочнице
  (в т.ч. когда саму песочницу не удалось инициализировать — Windows
  `CreateProcessAsUserW failed: 5`), он **не** перезапускает вне песочницы, а возвращает как
  сбой. Так закрыт fail-open путь «песочница не поднялась → выполнение без изоляции». Значение
  подставляется **литералом** (не переменной) — понижать/переопределять его нельзя. Флаг
  `-c` идёт после литерального префикса `codex exec` и на матч гранта не влияет.
...
- `codex exec` неинтерактивна по определению («Run Codex non-interactively») — отдельного
  CLI-**флага** режима одобрения ей не нужно (в актуальной `codex exec --help`, `codex-cli`
  0.142.5, `--ask-for-approval` отсутствует и приводит к ошибке разбора аргументов, exit 2).
  Но неинтерактивность сама по себе **не** делает исполнение fail-closed: с дефолтной
  `approval_policy` (`on-request`/`on-failure`) отказ песочницы молча эскалирует команду к
  запуску без изоляции. Поэтому политику одобрения задаём не флагом, а **config-оверрайдом**
  `-c approval_policy=never` (он `codex exec` принимает — это ключ конфига, а не CLI-флаг) на
  каждом вызове (см. выше). Автономность и границу правок обеспечивает связка `--sandbox
  workspace-write` + `-c approval_policy=never`: правки только в worktree, и никакого
  повышения режима вне песочницы при её сбое. Исходящая сеть — по ключу `CODEX_NETWORK` (при
  `on` открыта, при `off` заблокирована; см. «Конфигурация codex» и «Вызов codex»). Старые
  версии codex, где `exec` ещё принимал `--ask-for-approval never` флагом, этой формой **не
  поддерживаются** (эквивалент задаётся config-оверрайдом выше).
