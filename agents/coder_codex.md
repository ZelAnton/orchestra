...
| `tool-missing` | `Python was not found; run without arguments to install from the Microsoft Store` / `App execution aliases` (Windows App Execution Alias-заглушка вместо реального интерпретатора), либо `CreateProcessAsUserW failed: 2` (бинарь не найден по имени) | требуемый интерпретатор/инструмент отсутствует на хосте или подменён Store-алиасом — повтор не лечит (найдено аудитом T-067) |
