...
Вызов обязан передать `CALLER=processor|manual`. При `CALLER=manual` сначала выполни
read-only `state-tx.ps1 status --work "$WORK"`. Если lease live, не конкурируй с processor:
заверши без graph mutation и сообщи, что processor сам выполнит refresh на безопасной
границе. Stale/отсутствующий lease не усыновляй и не удаляй. При `CALLER=processor` считай
lease уже проверенным родителем; сам им не управляй. Не принимай неизвестный CALLER.
...
Не сканируй системный диск и не ищи соседние репозитории вне registry. Checkout-раннеры
...
1. Выполни `project-registry.ps1 list --json` и найди текущий проект по каноническому
   ...
   До аудита зафиксируй его числовой `graph_generation` как CAS-базу candidate.
...
5. Создай уникальный файл
   `$WORK/dependency_graph_candidates/depgraph-<UTC>-<random>.json` (никогда не общий
   фиксированный filename) в точной форме:

```json
{
  "schema": "orchestra/project-graph-snapshot@1",
  "base_graph_generation": 7,
...
6. Вызови `project-registry.ps1 graph-sync --root "$ROOT" --snapshot-file
   "<unique-candidate-path>" --json`. Команда атомарно заменяет только graph
   текущего проекта, проверяет upstream ids и идемпотентна при неизменном snapshot.
   Если другой refresh уже изменил graph generation, меняющий stale candidate получает
   CAS-отказ: не повторяй старый файл, а заново прочитай registry и пересобери аудит.
7. Удали только свой unique candidate после успешного sync. При ошибке оставь его как
   диагностику и ничего не
