...
$binName = if ($script:OnWindows) { 'orchestra-engine-spike.exe' } else { 'orchestra-engine-spike' }
...
if (-not (Test-Path -LiteralPath $script:EngineBin)) {
    ...
    Write-Host 'Building the engine crate once (cargo build --locked --bin orchestra-engine-spike)...'
    $build = Invoke-Proc 'cargo' @('build', '--locked', '--bin', 'orchestra-engine-spike') $script:EngineDir
