...
rem  - .claude\settings.local.json: ensures the canonical Codex exec-grant allow-list
...
%CC_PS_EXE% -NoProfile -Command "$ErrorActionPreference='Stop'; $p='.claude\settings.local.json'; $rules=@('Bash(codex exec *)'); $enc=New-Object System.Text.UTF8Encoding($false); if (Test-Path $p) { $raw=[System.IO.File]::ReadAllText($p,[System.Text.Encoding]::UTF8); $missing=@($rules | Where-Object { -not $raw.Contains($_) }); function Report($m){ Write-Host $m; foreach ($x in $missing) { Write-Host ('  - '+$x) }; exit 0 }; if ($missing.Count -eq 0) { Write-Host 'OK   .claude\settings.local.json already allows codex exec - left unchanged.'; exit 0 }; try { $j=$raw | ConvertFrom-Json } catch { $j=$null }; if ($null -eq $j) { Report '.claude\settings.local.json exists but is not valid JSON - cannot merge automatically; add the allow-rule(s) by hand to permissions.allow:' }; $pp=$j.PSObject.Properties['permissions']; if ($pp -and $null -ne $pp.Value -and -not ($pp.Value -is [PSCustomObject])) { Report '.claude\settings.local.json exists but its permissions is not a JSON object - cannot merge automatically; add the allow-rule(s) by hand to permissions.allow:' }; if (-not $pp -or $null -eq $pp.Value) { if ($pp) { $j.permissions=[PSCustomObject]@{} } else { $j | Add-Member -NotePropertyName permissions -NotePropertyValue ([PSCustomObject]@{}) } }; $perm=$j.permissions; $ap=$perm.PSObject.Properties['allow']; if ($ap -and $null -ne $ap.Value -and -not ($ap.Value -is [System.Array])) { Report '.claude\settings.local.json exists but its permissions.allow is not a JSON array - cannot merge automatically; add the allow-rule(s) by hand to permissions.allow:' }; if (-not $ap -or $null -eq $ap.Value) { if ($ap) { $perm.allow=@() } else { $perm | Add-Member -NotePropertyName allow -NotePropertyValue @() } }; $perm.allow=@($perm.allow)+$missing; $json=$j | ConvertTo-Json -Depth 20; $tmp=$p+'.tmp'; try { [System.IO.File]::WriteAllText($tmp,$json,$enc); [System.IO.File]::Replace($tmp,$p,[NullString]::Value) } catch { if (Test-Path $tmp) { Remove-Item -LiteralPath $tmp -Force -ErrorAction SilentlyContinue }; Report '.claude\settings.local.json could not be written (read-only or locked?) - cannot merge automatically; add the allow-rule(s) by hand to permissions.allow:' }; Write-Host ('Merged allow-rule(s) into .claude\settings.local.json permissions.allow: '+($missing -join ', ')+' (lets coder_codex/reviewer_codex run codex exec autonomously).'); exit 0 }; if (-not (Test-Path '.claude')) { New-Item -ItemType Directory -Path '.claude' -Force | Out-Null }; $o=[PSCustomObject]@{ permissions = [PSCustomObject]@{ allow = @($rules) } }; $json=$o | ConvertTo-Json -Depth 20; [System.IO.File]::WriteAllText($p, $json, $enc); Write-Host ('Created .claude\settings.local.json with allow-rule(s): '+($rules -join ', ')+' (lets coder_codex/reviewer_codex run codex exec autonomously).')"
goto :eof
