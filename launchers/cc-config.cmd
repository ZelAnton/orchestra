...
rem wrapper `pwsh -File tools/codex-runtime.ps1` the adapters actually run, plus the
...
rem "Codex-агенты" / "Разрешение на запуск codex"). TWO rules (task F-05):
rem Bash(pwsh -File tools/codex-runtime.ps1 *) - the command Bash ACTUALLY runs since
rem T-075 (the adapters drive codex through the runtime wrapper; the child `codex exec`
rem is spawned by the runtime past the Bash permission gate, so THIS rule is the one that
rem authorizes the real call, and it covers every runtime subcommand and every CODEX_CMD);
