...
# launcher process makes Codex's sandbox spawn fail; raw codex exec under a
# default approval_policy would then run unsandboxed, but Orchestra's adapters
# pin -c approval_policy=never per T-069, so within Orchestra it escalates
# ENV_LIMIT/sandbox-init and falls back to Claude instead of running
# unsandboxed). Neither concept (Windows process elevation, nor
