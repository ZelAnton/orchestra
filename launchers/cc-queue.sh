#!/usr/bin/env bash
# Run queue_builder in the current folder (Claude Code, acceptEdits mode).
...
if [ "$#" -eq 0 ]; then
  exec claude --agent queue_builder --permission-mode acceptEdits "Per your system prompt, ask me for the source (file/spec/backlog) or task description to enqueue into .work/Tasks_Queue.md - none was given on the command line."
else
  exec claude --agent queue_builder --permission-mode acceptEdits "Per your system prompt, add tasks to .work/Tasks_Queue.md. Task source or description: $*"
fi
