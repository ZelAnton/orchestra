#!/usr/bin/env bash
# Run code_auditor in the current folder through Claude or the interactive Codex TUI.
...
case "$PROVIDER" in
  claude)
    ...
    exec claude --agent code_auditor --permission-mode "$CLAUDE_PERMISSION_MODE" "Per your system prompt, audit the repository source code and enqueue each issue you find as a separate task in .work/Tasks_Queue.md. Start now."
    ;;
  codex)
    ...
    exec pwsh -NoProfile -File "$CODEX_ROLE_RUNTIME" -Role code_auditor -Root "$PROJECT_ROOT" -RequestedProvider codex
    ;;
