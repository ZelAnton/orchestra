...
- `MAX_PARALLEL` — сколько задач максимум исполняется одновременно в одном батче.
  ...
  `enhancement_scout`) можно смело держать на 5 и выше: накладные расходы батча
  (вызов planner, интеграционная ветка, merger, полное интеграционное ревью)
  амортизируются на большее число задач. Для тяжёлых/взаимозависимых задач имеет
  смысл понижать. Ограничители — способность planner найти столько
  непересекающихся доменов и «хвостовая» задержка самой медленной задачи батча.
