...
- `cc-processor`/`cc-resume` создают изолированное окружение сборок для обоих provider:
  ...
  (ввод не теряется) и записывает `containment_degraded_reason=processkit-cli-no-mediated-stdin`.
  Для интерактивного root одного stdin недостаточно: требуются унаследованные stdin/stdout/
  stderr с сохранением terminal semantics. `run:--inherit-stdio` и `--stdin-file` запрошены
  upstream; до появления первого surface Claude root идёт direct fallback, Codex root и leaf
  supervision продолжают использовать CLI.
