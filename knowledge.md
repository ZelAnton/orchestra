...
- **Fail-closed валидация значений Codex-ключей (T-072).** Шесть ключей —
  ...
  `config.example.md`; её копии несут `cc-doctor.cmd`/`cc-doctor.sh` (mirror-совместимость:
  `tools/` и `config.example.md` вне репозитория недоступны) и ветвление `processor.md`
...
- **Разрешение на запуск codex — предвыдаётся, а не выпрашивается по ходу.** В auto-режиме
  ...
  **префиксное** правило, поэтому адаптеры зовут codex **отдельной командой, начинающейся
  ...
  `.claude/settings*` для самопредоставления прав. Комплементарно: `cc-config` обеспечивает
  **постоянный** канонический allow-список (мин. `Bash(codex exec *)`) в
  ...
  `CC_CODEX_EXEC_GRANT=codex exec`; при его наличии (когда он покрывает фактический префикс
  команды `<CODEX_CMD> exec`) постоянное allow-правило не требуется и статический поиск не
  запускается. Иначе проверяется строго массив `permissions.allow` settings-файлов
  ...
  сверяется по фактическому префиксу `<CODEX_CMD> exec` — канонический `codex exec` за него
  ложного `OK` не даёт. Запасной путь, если версия Claude Code всё равно откажет, —
