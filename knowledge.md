...
2. **Зеркало `cc-sync`** (типовой mirror-only целевой проект — использует orchestra только
   ...
   `~/.claude/scripts/<script>.ps1`. Держи **тильду литеральной** в тексте команды (не
   подставляй заранее раскрытый `$HOME`): тильду раскрывает сама shell при выполнении, а
   неизменный литеральный текст с тильдой — это то, под что заведены allow-правила `cc-config`
   (важно для codex-раннера; см. ниже).
