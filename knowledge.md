...
- **Аренда владельца (`.work/orchestrator.lock`) мутируется только через `tools/state-tx.ps1`**
  ...
  считает contention только I/O-ошибку CreateNew при реально существующем lock-файле; если
  файл отсутствует повторно или тип ошибки иной, немедленно пробрасывает исходную I/O-причину,
