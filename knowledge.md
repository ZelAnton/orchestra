...
- `cc-processor`/`cc-resume` создают изолированное окружение сборок для обоих provider:
  ...
  PATH, `off` отключает CLI. Resolver fail-closed проверяет `probe` schema 1, reserved band
  100–119 и run/control/list surfaces; legacy `CC_PROCESSKIT_PYTHON` используется только как
  fallback. Неверный явный backend — exit 10. Корневая сессия получает durable lifecycle
  `.work/processes/_processor/*.processkit.jsonl`; это внешний kernel-backed backstop (Job
  Object/cgroup/pgroup), а не project config.
  ...
  CLI 0.2.0 не имеет mediated stdin: при непустом supervisor `--stdin-text`/`--stdin-file`
  resolver использует настроенный Python fallback, а без него — старый PID/PGID backend
  (ввод не теряется) и записывает `containment_degraded_reason=processkit-cli-no-mediated-stdin`. Корневой CLI-run пока
  автономный/noninteractive; `--inherit-stdin` и `--stdin-file` запрошены upstream.
