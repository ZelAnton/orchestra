...
- `merger.md` — последовательно сливает готовые ветки в `_integration`, разрешает или
  карантинит конфликты, пишет `merge_report.md`. Переданный `SMOKE_CMD` обязателен после
  каждого нового merge и ещё раз на финальной вершине (включая resume с already-merged);
  отчёт фиксирует источник проверки и точную проверенную вершину, которые валидирует processor.
