...
- **Единый исполняемый runtime `tools/codex-runtime.ps1` (T-075).** Механическая часть
  ...
  **безопасная очистка только своей рабочей копии** (`cleanup`; в основном дереве Фазы 5.4 —
  ...
  готовы к POSIX-прогону. **Зеркалирование в другие проекты (T-114).** `tools/sync-runtime.ps1`
  мирроит `codex-runtime.ps1` в `<dest>/scripts` тем же способом, что и `doctor-runtime.ps1`,
  так что `cc-sync` кладёт в целевой проект собственную копию
  `~/.claude/scripts/codex-runtime.ps1`; адаптеры резолвят путь к раннеру по обеим
  раскладкам (чекаут/зеркало — см. «Разрешение на запуск codex — предвыдаётся, а не
  выпрашивается по ходу» ниже и `agents/coder_codex.md`, «Резолвинг пути к runtime»), иначе
  вне чекаута orchestra голый относительный путь `tools/codex-runtime.ps1` не резолвился бы
  вовсе.
